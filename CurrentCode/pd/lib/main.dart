...
class ViewListPageState extends State<ViewListPage>
  ...
  Future<void> _addScannedItemsToFirestore(
    ScannedItemsModel scannedItemsModel,
  ) async {
    ...
    for (String scannedItem in scannedItemsModel.scannedItems) {
      ...
      await _createExcelFile(
        scannedItemsModel.scannedItems,
        barcodeToInfoMap,
      );
    ...
    final excelPath = '${directory.path}/scanned_items.xlsx';
    final file = File(excelPath);

    try {
      await storageReference.putFile(file);
      print('Excel file uploaded to Firebase Cloud Storage.');
    } catch (error) {
  ...
  Future<void> _createExcelFile(
    ...
    sheet.appendRow(['Name', 'Barcode Number', 'Quantity', 'Price', 'Weight']);
    ...
    for (String scannedItem in scannedItems) {
      ...
      sheet.appendRow([itemName, scannedItem, price, weight]);
    }
